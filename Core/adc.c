...
CircularQueue pump_adc_queue;
uint32_t channels[] = {PUMP_ADC_CH};
static uint32_t adc_gather_value[sizeof(channels) / sizeof(channels[0])];

void adc_gpio_init(void)
{
    PUMP_ADC_GPIO_CLK_ENABLE();
    LL_GPIO_SetPinMode(PUMP_ADC_GPIO_PORT, PUMP_ADC_PIN, LL_GPIO_MODE_ANALOG);
}

void adc_queue_init(void)
{
    initQueue(&pump_adc_queue);
}
...
void adc_gather_handle(void)
{
    ...
    if (LL_GetTickDiff(adc_gather_interval) >= 100)   
    {
        for (size_t i = 0; i < sizeof(channels) / sizeof(channels[0]); i++)
        {
            ...
            if (i == 0)
            {
                enqueue(&pump_adc_queue, adc_gather_value[i]);
            }       
...
{ 
    return movingAverage(&pump_adc_queue);
}
